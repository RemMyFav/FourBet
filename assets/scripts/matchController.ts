...
export class matchController extends Component {
    ...
    start() {
        this.matchBegin(this.seats).then(() => {
            this.winner(this.seats).then((res)=>{
                console.log(res)
            }
            );
            // console.log(this.seats)
    ...
    private async winner(seats: Node[]): Promise<Node[]>{
        const handTypes: HandType[] = await this.getHandTypes(this.seats);
        const ordering: HandType[] = await this.order(handTypes);
        const biggest: HandType = ordering[0];
        const winner: Node[] = await this.multiWinner(handTypes, biggest);
        return new Promise((reslove, reject) =>{
            reslove(winner);
        })
    }

    private multiWinner(handTypes: HandType[], biggest: HandType): Promise<Node[]> {
        let winners: Node[] = [];
      
        for (let i = 0; i < handTypes.length; i++) {
          if (
            handTypes[i].level === biggest.level &&
            handTypes[i].type === biggest.type &&
            handTypes[i].val === biggest.val
          ) {
    ...
    private order(handTypes: HandType[]): Promise<HandType[]>{
        let ordering: HandType[] = [];
        for(let j = 0; j < handTypes.length; j++){
            if(ordering.length === 0){
                ordering.push(handTypes[j]);
            }
            else{
                for(let i = 0; i < ordering.length; i++){
                    if(ordering[i].level < handTypes[j].level){
                        ordering.splice(i, 0, handTypes[j]);
                        break;
                    }
                    else if(ordering[i].level === handTypes[j].level){
                        if(ordering[i].val < handTypes[j].val){
                            ordering.splice(i, 0, handTypes[j]);
                            break;
                        }
                        else if(ordering[i].val === handTypes[j].val){
                            ordering.splice(i, 0, handTypes[j]);
                            break;
                        }
                        else{
                            ordering.splice(i + 1, 0, handTypes[j]);
                            break;
                        }
                    }
                    else{
    ...
    private async getHandTypes(seats: Node[]): Promise<HandType[]>{
        ...
        for(let i = 0; i < seats.length; i++){
            const cardsArray: number[] = await this.cardValArray(this.seats[i]);
            const thisType: HandType = await this.getType(cardsArray);
            handTypes.push(thisType);
        }
    ...
    private cardsArrayToMap(cardsArray: number[]): Promise<Map<number, number>>{
    ...
    private isThreeKindOrPair(cardsMap: Map<number, number>): null | HandType {
        for (const [cardFace, num] of cardsMap.entries()) {
          if (num === 3) {
            const result = new ThreeKind();
            result.val = cardFace;
            return result;
          } else if (num === 2) {
      ...
      private isFlush(cardsMap: Map<number, number>): null | HandType {
        for (const [cardFace, num] of cardsMap.entries()) {
          if (num === 1) {
            const nextOneCard = cardFace + 1;
            const nextTwoCard = cardFace + 2;
    ...
    private cardValArray(seat: Node): Promise<number[]>{
        const cards: Node[] = seat.getComponent(SeatController).getCards();
        let result: number[] = [];
        for(let i = 0; i < cards.length; i++){
            result.push(cards[i].getComponent(CardController).getCardFace())
        }
        return new Promise((reslove, reject) =>{
     ...
     */
    private generateDeck(): Promise<void>{
     ...
     * @returns Promise<Node[]>
     */
    private generateSeat(): Promise<Node[]>{
        const seats: Node[] = [];
        for(let i = 0; i < this.seatNum; i++){
            const seatNode: Node = new Node("SEAT" + i);
     ...
     * @returns Promise<void>
     */
    private async matchBegin(seats: Node[]): Promise<void>{
        const a = await this.deck.getComponent(DeckController).dealCard(seats);
        console.log(a)
        return new Promise((reslove, reject) => {
            reslove();
        })
